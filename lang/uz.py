lang = {
    'start' : '''🤖 Botga xush kelibsiz, {}

📊Men Guruhga kim qancha odam qo'shganligini aytib beruvchi botman. 

Bot orqali Guruhingizga istagancha odam yigʻib olasiz, chunki odam qo'shmasa yozishni cheklaydi.

/help  -  buyrugi orqali bot buyruqlari haqida ma'lumot olishingiz mumkin☑️

⚠️ Bot to'g'ri ishlashi uchun ADMIN huquqini berishingiz kerak''',
'help': '''🤖 Botimizning buyruqlari! 

/mymembers - 📊Siz qo'shgan odamlar soni!
______
/yourmembers - 📈Reply qilingan odamning , guruhga qo'shgan odamlar soni!
______
/top  -  🏆Eng ko'p odam qo'shgan 10 talik!
______
/delson -  🗑Guruhga odam qo'shganlarni barchasini tozalash!
______
/clean -  🧹Reply qilingan habar egasini malumotlarini 0 ga tenglash!

👥 Guruhga odam yigʻish buyruqlari

/add -  buyrug'i Guruhingizga majburiy odam qo'shishni yoqadi!. Bu orqali Guruhingizga istagancha odam yigʻib olasiz.
_____
/add 10 - majburiy odam qo'shishni yoqish!

❗️eslatma: 10 soni o'rniga istagan raqamizni yozib jonatishiz mumkin!
_____
/add off  - majburiy odam qo'shishni o'chirib qoyish uchun!

/chanoff 

/chanon''',
    'bot_join':'''📊Men Guruhga kim qancha odam qo'shganligini aytib beruvchi botman. 

Bot orqali Guruhingizga istagancha odam yigʻib olasiz vedio qoʻllanmada koʻrsatilgan Botni ishlatish. 

/help  -  buyrugi orqali bot buyruqlari haqida ma'lumot olishingiz mumkin☑️

⚠️ Bot to'g'ri ishlashi uchun ADMIN huquqini berishingiz kerak''',
    'bot_admin':'''BOT -  GURUHGA ADMIN QILINDI ✅

👮🏻‍♂️ Men bu Guruhda ishlashga tayyorman !''',
    'instaled_tar': "<code>{}</code> <b>guruhida majburiy odam qo'shish tizimi ishga tushdi\n\nTARIF: {}!</b>",
    'tarif_off':"<code>{}</code><br>guruhida majburiy odam qo'shish to'xtatildi!</b>",
    'no_admin':"<a href='tg://user?id={}'>{}</a><b> Ushbu buyruq faqat adminlar uchun ishlaydi!</b>",
    'no_add': "<a href='tg://user?id={}'>{}</a>\n❌ Siz hali odam qo'shmagansiz!",
    'yes_add': "<a href='tg://user?id={}'>{}</a>\n🔹 Siz {} ta odam qo'shgansiz!",
    'ryes_add': "<a href='tg://user?id={}'>{}</a>\n🔹 {} ta odam qo'shgan!",
    'rno_add': "<a href='tg://user?id={}'>{}</a>\n❌ hali odam qo'shmagan!",
    'no_rep': "<b>siz guruhga kim qancha odam qo'shganligini bilish uchun usha odamga (reply) qilib , </b>/yourmembers <b>so'zini junatishingiz kerak!</b>",
    'top': "👥 <code>{}</code><b> guruhiga odam qo'shgan TOP 10 talik:</b>{}",
    'top_nad': "<a href='tg://user?id={}'>{}</a> <b>Ushbu buyruq faqat adminlar uchun ishlaydi!</b>",
    'delson': "<code>{}</code> ✅ <b>guruhida barcha odam qo'shganlar soni tozalandi! 🗑</b>",
    'user_cl':"<a href='tg://user?id={}'>{}</a>ni Guruhdagi ma'lumoti tozalandi! 🗑",
    'adam_qos': "<b>Kechirasiz!</b> <a href='tg://user?id={}'>{}</a> <b>siz guruhga yozish uchun avval {} ta odam qo'shishingiz zarur!</b>",
    'no_btn': "❌Ushbu tugma siz uchun emas",
    'btnn': "To'liq odam qo'shmadingiz, siz yana guruhga {} ta odam qo'shishingiz zarur!",
    'noadminbot':"<a href='tg://user?id={}'>{}</a> <b>Ushbu buyruqdan foydalanish uchun botni guruhga to'liq admin qilishingiz kerak!</b>",
    'xadd': "Xato!\n\nNamuna: <code>/add 10</code>",
    'supGR':'Bot faqat "SUPERGROUP"da ishlaydi',
    'tarif_off': 'Majburiy azo qoshish o\'chirildi'
}